...
namespace cxx::elog
{
  ...
  int Elog::SubmitElog()
  {
    ...
    if(!m_Textfile.empty())
    {
      ...
      char text2[100000];
      const int fh = open(m_Textfile.c_str(), O_RDONLY | O_BINARY);
      if(fh < 0)
      {
    ...
    {
      const int fh = open(m_Attachments[i].c_str(), O_RDONLY | O_BINARY);
      if (fh < 0)
      {
  ...
  buffer.push_back(std::vector<std::byte>(att_size[i]));

  int n = read(fh, &buffer[i][0], att_size[i]);
  if (n < att_size[i])
  {
    std::cout<<"Cannot fully read attachment file \""<<m_Attachments[i].c_str()<<"\".\n";
  return 1;
  }
  //buffer[i][n] = static_cast<std::byte>(0);
...
//oss << "---------------------------" << std::uppercase << std::setfill('0')<<std::hex << std::setw(4) << a  << b <<c<<"\n";*/



   sprintf(boundary, "---------------------------%04X%04X%04X",dist(gen), dist(gen),dist(gen));
    ...
    for(std::size_t i = 0; i < m_Attachments.size(); i++)
    {
      ...
      p += strlen(p);
      memcpy(p, &buffer[i][0], att_size[i]);
      p += att_size[i];
      strcpy(p, boundary);
      strcat(p, "\r\n");
    ...
    if(!m_Subdir.empty()) request+= m_Subdir+"/";
    if(!m_Logbook.empty()) request+= url_encode(m_Logbook)+"/  HTTP/1.0\r\nContent-Type: multipart/form-data; boundary="+boundary+"\r\n";
    if(getPort()!= 80) request+=std::string(getHostname())+':'+std::to_string(getPort());
    else request+=std::string(getHostname());
    request+="Host: "+std::string(str)+"\r\n";
    request+="User-Agent: ELOG\r\n";
    request+="Content-Length: "+std::to_string(content_length)+"\r\n\r\n";
    request.reserve(request.size() + content_length);
    request.append(content, content_length);
  ...
  std::string Elog::retrieve_elog(std::map<std::string,std::string>& attrib,std::string& text)
  {
    ...
    //std::cout<<"Response :\n"<<resp<<"\n ENDRESPONSE"<<std::endl;

    m_Connector.disconnect();
    /* check response status */
    const std::size_t start=resp.find("$@MID@$:");
    if(start==std::string::npos && m_ID==-1) return "-1";
