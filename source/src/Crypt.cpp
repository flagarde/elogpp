...
static char *sha256_crypt_r(const char *key, const char *salt, char *buffer, int buflen)
{
   ...
   if ((key - (char *) 0) % sizeof(uint32_t) != 0) {
     ...
     key = copied_key = static_cast<char*>(memcpy(tmp + sizeof(uint32_t)
     - (tmp - (char *) 0) % sizeof(uint32_t), key, key_len));
   }

   if ((salt - (char *) 0) % sizeof(uint32_t) != 0) {
     char *tmp = (char *) alloca(salt_len + sizeof(uint32_t));
     salt = copied_salt = static_cast<char*>(memcpy(tmp + sizeof(uint32_t)
     - (tmp - (char *) 0) % sizeof(uint32_t), salt, salt_len));
   }
   ...
   cp = p_bytes = static_cast<char*>(alloca(key_len));
   for (cnt = key_len; cnt >= 32; cnt -= 32) {
   ...
   cp = s_bytes = static_cast<char*>(alloca(salt_len));
